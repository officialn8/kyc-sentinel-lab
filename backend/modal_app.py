...
gpu_image = (
    ...
    .apt_install(
        ...
        "libgl1-mesa-glx",
        "libglib2.0-0",
        # Build tools for InsightFace Cython extensions
        # Need clang++ because NVIDIA image's Python was compiled with clang
        "build-essential",
        "clang",
    )
    .pip_install(
        # Core ML libraries (no GPU needed for these)
        "numpy>=1.24.0,<2.0.0",
    ...
    .pip_install(
        # Face analysis - GPU-accelerated via onnxruntime-gpu
        # CUDA 12.4 runtime image has cuBLAS 12 and cuDNN 9 pre-installed
        "insightface>=0.7.3",
...
def analyze_face(
    ...
    # Initialize InsightFace with GPU preference
    # Will use CUDA if available, falls back to CPU
